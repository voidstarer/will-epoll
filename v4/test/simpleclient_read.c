...
static void run_client(const char *hostname, int32_t server_port, uint16_t client_id, uint16_t dst_id)
{
	...
	n = snprintf((void*)packet->data, BUFSIZE-PACKET_HDR_LENGTH, "Hello from client %hu to %hu", client_id, dst_id);
	SET_PACKET_DATA_LENGTH(packet, n);

        fprintf(stdout, "sending hello packet: type: %u src_id:%hu dst_id:%hu length: %lu total_length=%u count=%d\n",
                GET_PACKET_TYPE(packet), GET_PACKET_SRC_ID(packet),
                GET_PACKET_DST_ID(packet), GET_PACKET_DATA_LENGTH(packet), GET_PACKET_LENGTH(packet), 1);
	...
	while(1) {
		...
		n = read(sockfd, buf+rc, BUFSIZE-rc);
		if (n <= 0) 
		...
		for(;;) {
			...
			fprintf(stdout, "type: %u src_id:%hu dst_id:%hu length: %u total_length=%u msg=%.*s\n",
				GET_PACKET_TYPE(packet), GET_PACKET_SRC_ID(packet),
